...
const CampaignDetailsDonateAllComent = (dataDonorAll) => {
  ...
  const UserAllComent = dataDonorAll.dataDonorAll?.dataDonateComen
    ?.slice(0, limit)
    .map((item, index) => {
      return (
        <div className={styles.card} key={index}>
          <div className={styles.cardheader}>
            <div className={styles.foto}>
              <img
                src={item?.User?.photo ? item?.User?.photo : cardImg1}
                alt="user"
              ></img>
            </div>
      ...
      <div className={styles.container}>
        ...
        <div className={styles.btnwrap}>
          ...
          {cekisi === 0 ? (
            ...
            <button onClick={loadMore} className={styles.btnLoadmore}>
              LOAD MORE
            </button>
          ) : (
            <button onClick={loadLess} className={styles.btnLoadmore}>
              SHOW LESS
            </button>
          )}
        </div>
